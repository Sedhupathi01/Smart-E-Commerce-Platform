        ...
        <Card className="border-slate-800/60 bg-slate-900/60 backdrop-blur-2xl shadow-2xl overflow-hidden ring-1 ring-white/10">
          ...
          <CardHeader className="space-y-3 pb-8 pt-10 px-10 text-center">
            ...
            <CardDescription className="text-slate-400 font-medium text-sm max-w-[250px] mx-auto leading-relaxed">
                Join the definitive standard of premium e-commerce.
            </CardDescription>
          </CardHeader>
          <CardContent className="px-10 pb-10">
            <form onSubmit={handleRegister} className="space-y-5">
              <div className="space-y-4">
                ...
                <div className="relative group">
                  <Mail className="absolute left-3.5 top-3.5 h-4 w-4 text-slate-500 group-focus-within:text-violet-400 transition-colors" />
                  <Input
                    type="email"
              ...
              <Button
                type="submit"
                className="w-full h-12 bg-white hover:bg-slate-200 text-slate-950 font-bold tracking-wide rounded-xl transition-all duration-300 transform active:scale-[0.98] group mt-4 shadow-[0_0_20px_rgba(255,255,255,0.1)]"
                disabled={loading}
              >
                {loading ? (
          ...
          <CardFooter className="flex flex-col space-y-4 pb-8 bg-slate-900/30 border-t border-slate-800/50 pt-6">
            <div className="text-sm text-center text-slate-500 flex flex-col gap-2">
              <span>Already part of Aura?</span>
              <Link href="/auth/login" className="text-white hover:text-violet-300 font-bold transition-colors uppercase tracking-widest text-xs flex items-center justify-center gap-1 group">
                Enter Here <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
          </CardFooter>
        </Card>
